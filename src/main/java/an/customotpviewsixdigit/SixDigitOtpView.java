package an.customotpviewsixdigit;
...
/**
 * Created by sahitya on 22/8/17.
 */

public class SixDigitOtpView extends LinearLayout {
    ...
    public SixDigitOtpView(Context context) {
        super(context);
        init(null);
    }

    public SixDigitOtpView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public SixDigitOtpView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray styles = getContext().obtainStyledAttributes(attrs, R.styleable.SixDigitOtpView);
        LayoutInflater mInflater =
                (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mInflater.inflate(R.layout.otpview_layout, this);
        mOtpOneField = (EditText) findViewById(R.id.first_edit_text);
        mOtpTwoField = (EditText) findViewById(R.id.second_edit_text);
        mOtpThreeField = (EditText) findViewById(R.id.third_edit_text);
        mOtpFourField = (EditText) findViewById(R.id.fourth_edit_text);
        mOtpFiveField = (EditText) findViewById(R.id.fifth_edit_text);
        mOtpSixField = (EditText) findViewById(R.id.sixth_edit_text);
    ...
    /**
     * Checks if all six fields have been filled
     *
    ...
    public void setOTP(String otp) {
        if (otp.length() != 6) {
            Log.e("OTP", "Invalid otp param");
            return;
        }
        if (mOtpOneField.getInputType() == InputType.TYPE_CLASS_NUMBER
                && !otp.matches("[0-9]+")) {
            Log.e("OTP", "OTP doesn't match INPUT TYPE");
            return;
        }
    ...
    private void styleEditTexts(TypedArray styles) {
        int textColor = styles.getColor(R.styleable.SixDigitOtpView_android_textColor, Color.BLACK);
        int backgroundColor =
                styles.getColor(R.styleable.SixDigitOtpView_background_color, Color.TRANSPARENT);
        if (styles.getColor(R.styleable.SixDigitOtpView_android_textColor, Color.TRANSPARENT)
                != Color.TRANSPARENT) {
    ...
    private void setEditTextInputStyle(TypedArray styles) {
        int inputType =
                styles.getInt(R.styleable.SixDigitOtpView_android_inputType, EditorInfo.TYPE_TEXT_VARIATION_NORMAL);
        ...
        String text = styles.getString(R.styleable.SixDigitOtpView_otp);
    ...
    /*
        This will return the edittext which is currentely focused by the user while entering OTP
     */
    ...
    /*
        This will return the current entering Edittext
     */

    public void currentEidtText() {
